...
class Course extends Controller{
    ...
    public function index(){
        ...
        if (Cookie::get('search_course') == "") {
            ...
            $data = DB::table('course')->select('course.id','name','title','course.status','course.created_at','course.updated_at')->join('course_category','course_category.id','=','course.course_category_id')->orderBy('course.id', 'desc')->paginate(15);
        } else {
            // if cookie is existed
            $data = DB::table('course')->select('course.id','name','title','course.status','course.created_at','course.updated_at')->join('course_category','course_category.id','=','course.course_category_id')->where("name", "like", '%' . Cookie::get('search_course') . '%')->orderBy('id', 'desc')->paginate(15);
        }
        $data->setPath('course');
    ...
    //     $course_category = new Course_Model;
    ...
    //     $this->validate($request, ["title" => "required"], ["title.required" => "Vui lòng nhập tiêu đề"]);
    //     $course_category = Course_Model::find($id);
    //     $course_category->title = $request->title;
    //     $course_category->status = $request->status;
    //     if ($course_category->save()) {
    ...
    //         $course_category = Course_Model::find($list_id[0]->id);
    ...
    //             $course_category = Course_Model::find($value->id);
    //             $course_category->status = 2; //2 is trash
