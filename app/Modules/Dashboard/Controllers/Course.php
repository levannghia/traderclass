...
class Course extends Controller{
    ...
    public function index(){
        ...
        if (Cookie::get('search_course') == "") {
            ...
            $data = DB::table('course')->select('course.id','name','title','course.status','course.created_at','course.updated_at','teachers.fullname')->join('course_category','course_category.id','=','course.course_category_id')->join('teachers','teachers.id','=','course.teacher_id')->orderBy('course.id', 'desc')->paginate(15);
        } else {
            // if cookie is existed
            $data = DB::table('course')->select('course.id','name','title','course.status','course.created_at','course.updated_at','teachers.fullname')->join('course_category','course_category.id','=','course.course_category_id')->join('teachers','teachers.id','=','course.teacher_id')->where("name", "like", '%' . Cookie::get('search_course') . '%')->orderBy('id', 'desc')->paginate(15);
        }
        $data->setPath('course');
        $row = json_decode(json_encode([
            "title" => "Course - Danh sách",
        ]));

        return view("Dashboard::course.index", compact("row", "data"));
    }

    public function postIndex(Request $request) {
        if (isset($_POST["btn_search"])) {
            Cookie::queue("search_course", $request->search, 60);
            return redirect()->route("admin.course")->with(["type" => "success", "flash_message" => "Tìm kiếm : " . $request->search]);
        }
    }
    ...
    public function postAdd(Request $request){
        $settings = config('global.settings');
        $this->validate($request, ["name" => "required", "photo" => "required", "link_video" => "required"], ["name.required" => "Vui lòng nhập tên khóa học", "photo.required" => "Vui lòng chọn hình ảnh", "link_video.required" => "Vui lòng nhập link video"]);
        $course = new Course_Model;
        $course->name = $request->name;
        $course->link_video = $request->link_video;
